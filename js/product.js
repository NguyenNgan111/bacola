...
import { hotProductForThisWeek } from "../data/hotProductForThisWeek.js";
import { trendingProducts } from "../data/trendingProducts.js";
import { categories } from "../data/categories.js";
import { superDiscountWeekend } from "../data/superDiscountWeekend.js";
import { subBanner } from "../data/subBanner.js";
export function newProductHandler() {
  ...
  const newProduct = products
    ...
    .join("");
  $(".new-product .container .row").html(newProduct);
  addToCart();
...
export function hotProductThisWeekHandler() {
  ...
  const hotItem = products
    .filter((product) =>
      hotProductForThisWeek.find((id) => id.productId == product.id)
    )
    .map((item) => {
      return `
                <a href="">
        ...
        item.price
                    ...
                    <div class="product-expired">
                        <div class="countdown" data-date="${
                          hotProductForThisWeek[0].timeExpired
                        }">
                            ${countDownTimer(
                              hotProductForThisWeek[0].timeExpired,
                              hotProductForThisWeek[0].timeStart
                            )}
                        </div>
...
export function superDiscountWeekendHandler() {
  ...
  const html = products
    .filter((product) =>
      superDiscountWeekend.find((item) => item.productId == product.id)
    )
    .forEach((element) => {
            ...
            <div class="col" data-id="${element.id}">
                <a href="">
                    <div class="banner-content">
                        <h6 class="content-header">${
                          superDiscountWeekend[i].text
                        }</h4>
                    <div class="content-main">
                        <h3>${element.title}</h3>
                        <p>${superDiscountWeekend[i++].discription}</p>
                    </div>
